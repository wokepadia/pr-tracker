  ...
  const statusItems = [
    {
      id: "review",
      label: "Review state",
      value: userReviewStanding(item.userLastReviewDecision),
    ...
    },
    {
      id: "unseen",
      label: "Unseen",
      value: `${formatCount(newEventCount, "new event")} since ${item.lastSeenAt}`,
      show: newEventCount > 0,
      hot: true,
    },
  ].filter((status) => status.show !== false)

  return (
    <section className="border-b border-border px-7 py-4">
      <div className="flex flex-wrap items-center gap-2 text-foreground">
        ...
        {statusItems.map((status) => (
          <span
            key={status.id}
            className={cn(
              "inline-flex h-8 items-center gap-1.5 rounded-md border border-border bg-card px-2.5 text-xs",
              status.hot && detailToneClasses[tone]
            )}
          >
        ...
        <div className="ml-auto text-xs text-muted-foreground">
          Timeline below is newest first
        </div>
      </div>
    </section>
  )
